  ...
  const UpdateXctrl = () => {
    if (isOpenInf && !flagEtalonInf) {
      ...
      if (pointsAdd.length > 0) {
        for (let i = 0; i < pointsAdd.length; i++) pointsEtalonXctrl.push(pointsAdd[i]);
      }
    }
    if (isOpenInf && flagEtalonInf) {
          ...
          <Box sx={{ width: '98.8%' }}>
            <TabContext value={value}>
              <Box sx={styleBoxTabContext}>
                {!bsLogin && <>{ButtonMenu('1', 'Управление', SetValue, tekValue)}</>}
                {!bsLogin && <>{ButtonMenu('2', 'Характерные точки', SetValue, tekValue)}</>}
