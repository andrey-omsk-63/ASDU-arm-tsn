...
const App = () => {
  ...
  const InputNewDateInterval = () => {
    ...
    if (value === '4') {
      ...
      if (formSett !== formSettToday) dat = MakeInterval(massIntervalOldStart[tekIdOld]);
    }
    ...
    return (
      <>
        ...
        <Grid item container sx={{ border: 0, width: '120px' }}>
          ...
          <Grid item xs={5}>
            <Box sx={styleInt01}>
              <InputInterval curr={currencies} cur={currency} func={ChangeInt} />
            </Box>
          </Grid>
        </Grid>
            ...
            <TabContext value={value}>
              <Box sx={{ marginLeft: 0.5, backgroundColor: '#F1F5FB' }}>
                <Stack direction="row">
                  ...
                  {!bsLogin && <>{ButtonMenu('2', 'Характерные точки', SetValue)}</>}
                  {ButtonMenu('3', 'Статистика', SetValue)}
                  {!bsLogin && value === '2' && saveXT && (
                    <>{ButtonMenu('7', 'Сохр.в файл', SetValue)}</>
                  )}
                  {/* {value === "2" && isOpenSt && <InputNewDateInterval />} */}
                  {value === '3' && isOpenSt && <InputNewDateInterval />}
                  {value === '4' && isOpenOldSt && !nullOldStatistics && <InputNewDateInterval />}
                </Stack>
              </Box>
              <TabPanel value="1">
