...
const App = () => {
    ...
    WS.onmessage = function (event: any) {
      ...
      switch (allData.type) {
        ...
        case "getStatistics":
          ...
          if (!data.statistics) {
            st = dataStatNow.data.statistics;
  ...
  if (debug && flagOpenDebug) {
    ...
    // console.log("@@@", dataStatNow);
    // SetStatisticsIntervalNow(dataStatNow.data.statistics);
    // setPointsSt(dataStatNow.data.statistics);
    // dispatch(statsaveCreate(datestat));
    // setIsOpenSt(true);
    //alert("На текущюю дату данных по стистке нет");
