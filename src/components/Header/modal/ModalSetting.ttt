...
const ModalSetting = () => {
  ...
  const handleCloseSetBut = () => {setOpenSet(false);
  };
  const styleSet = {
    position: 'absolute',
    top: '48%',
  ...
  const TextMaskCustom = React.forwardRef(function TextMaskCustom(props, ref) {
    ...
    return (
      <IMaskInput
        ...
        inputRef={ref}
        onAccept={(value: ChangeEvent<HTMLInputElement>) => onChange({ target: { name: props.name, value } })}
        overwrite
      />
    );
  });
  ...
  const InpNamePort = () => {
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
      setValuen(event.target.valueAsNumber);
      formSett[0].namePort = event.target.valueAsNumber;
    };
    return (
      <TextField
        size="small"
  ...
  const InpCodReg = () => {
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
      setValuen(event.target.valueAsNumber);
      formSett[0].codReg = event.target.valueAsNumber;
    };
    return (
      <TextField
        size="small"
